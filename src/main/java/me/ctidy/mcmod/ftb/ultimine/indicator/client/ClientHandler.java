...
import me.ctidy.mcmod.ftb.ultimine.indicator.FTBUltimineIndicatorNeoForge;
...
public class ClientHandler {
    ...
    public static final String SHAPE_ICON_PATH = FTBUltimineIndicatorNeoForge.MOD_ID + "/" + SHAPE_ICON_ID + "/";
    ...
    private static final ResourceLocation MISSING_ICON_ID = FTBUltimineIndicatorNeoForge.id(SHAPE_ICON_PATH + SHAPE_ICON_MISSING_ID);
    ...
    public static void bindIconSprites() {
        ...
        final int count = ShapeRegistry.INSTANCE.shapeCount();
        for (int i = 0; i < count; i++) {
            final String name = ShapeRegistry.INSTANCE.getShape(i).getName().getPath();
            TextureAtlasSprite sprite = atlas.getSprite(FTBUltimineIndicatorNeoForge.id(name).withPrefix(SHAPE_ICON_PATH));
    ...
    public static void renderHud(final GuiGraphics guiGraphics, final Window window, final float partialTicks) {
        final FTBUltimineClient ultimineClient = FTBUltimineClient.getInstance();
        if (!FTBUltimineIndicatorClientConfig.SHOW_INDICATOR.get()
                || ultimineClient == null
                || !(ultimineClient instanceof FTBUltimineClientAccessor accessor)
                || !accessor.isPressed()) {
            return;
        }

        final Minecraft mc = Minecraft.getInstance();
        final var shape = ShapeRegistry.INSTANCE.getShape(accessor.getShapeIdx());
        final String shapeName = shape.getName().getPath();  // Key of Map of SHAPE_ICONS, not the display name
        ...
        if (ShapeNameActiveMode.ALWAYS == mode
                || (ShapeNameActiveMode.ON_SCROLLABLE == mode && accessor.isSneak())) {
            final Component textShapeName = shape.getDisplayName();
        ...
        if (FTBUltimineIndicatorClientConfig.SHOW_ULTIMINE_STATUS.get()) {
            ...
            } else if (accessor.canUltimine() && accessor.getActualBlocks() > 0) {
                textStatus = Component.translatable("ftbultimine.info.blocks", accessor.getActualBlocks()).withStyle(Style.EMPTY.withColor(TextColor.fromRgb(0xA3BE8C)));
            } else {
            ...
            if (FTBUltimineIndicatorClientConfig.SHOW_CHANGE_SHAPE_HINT.get()) {
                final Component textHint = FTBUltimineClientConfig.REQUIRE_SNEAK_FOR_MENU.get()
                        ? Component.translatable("ftbultimine_indicator.hint.change_shape_scroll", mc.options.keyShift.getTranslatedKeyMessage())
                        : Component.translatable("ftbultimine_indicator.hint.change_shape_scroll.no_shift");
                final Positioning.ActualPosition hintPos = FTBUltimineIndicatorClientConfig.ULTIMINE_STATUS_POSITION.get().getPanelPos(
                        window.getGuiScaledWidth(), window.getGuiScaledHeight(),
                        mc.font.width(textHint), mc.font.lineHeight,
                        FTBUltimineIndicatorClientConfig.ULTIMINE_STATUS_INSET_X.get(), FTBUltimineIndicatorClientConfig.ULTIMINE_STATUS_INSET_Y.get() + mc.font.lineHeight + 2,
                        FTBUltimineIndicatorClientConfig.ULTIMINE_STATUS_ANCHOR_X.get(), FTBUltimineIndicatorClientConfig.ULTIMINE_STATUS_ANCHOR_Y.get()
                );
                guiGraphics.drawString(mc.font, textHint, hintPos.x(), hintPos.y(), 0xB0B7C3, true);
            }
        }
    }
